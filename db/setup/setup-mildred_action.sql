...
CREATE TABLE `action_dispatch` (
  ...
  `py_package` varchar(128) DEFAULT NULL,
  `py_module` varchar(128) NOT NULL,
  `py_class` varchar(128) DEFAULT NULL,
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_action_reasons_w_ids` AS select `at`.`id` AS `meta_action_id`,`at`.`name` AS `meta_action`,`at`.`priority` AS `action_priority`,`ad`.`id` AS `action_dispatch_id`,`ad`.`identifier` AS `action_dispatch_identifier`,`ad`.`category` AS `action_dispatch_category`,`ad`.`py_module` AS `action_dispatch_module`,`ad`.`py_class` AS `action_dispatch_class`,`ad`.`py_func` AS `action_dispatch_func`,`rt`.`id` AS `meta_reason_id`,`rt`.`name` AS `reason`,`rt`.`weight` AS `reason_weight`,`ad2`.`id` AS `conditional_dispatch_id`,`ad2`.`identifier` AS `conditional_dispatch_identifier`,`ad2`.`category` AS `conditional_dispatch_category`,`ad2`.`py_module` AS `conditional_dispatch_module`,`ad2`.`py_class` AS `conditional_dispatch_class`,`ad2`.`py_func` AS `conditional_dispatch_func` from ((((`meta_action` `at` join `action_dispatch` `ad`) join `action_dispatch` `ad2`) join `meta_reason` `rt`) join `action_reason` `ar`) where ((`at`.`dispatch_id` = `ad`.`id`) and (`rt`.`dispatch_id` = `ad2`.`id`) and (`at`.`id` = `ar`.`meta_action_id`) and (`rt`.`id` = `ar`.`meta_reason_id`)) order by `at`.`name` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
