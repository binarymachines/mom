...
def get_genre_folder_names():
    results = []
    rows = mySQL.retrieve_values('media_genre_folder', ['name'], [])
    for r in rows: results.append(r[0])
    return results

def get_active_media_formats():
    results = []
    rows = mySQL.retrieve_values('media_format', ['active_flag', 'ext'], ['1'])
    for r in rows: results.append(r[1])
    return results

def get_location_folder_names():
    results = []
    rows = mySQL.retrieve_values('media_location_folder', ['name'], [])
    for r in rows: results.append(r[0])
    return results
...
def setup_genre_folders():
    ...
    for f in folders:
        # print f
        rows = mySQL.retrieve_values('media_genre_folder', ['name'], [f.lower()])
        if len(rows) == 0:
            mySQL.insert_values('media_genre_folder', ['name'], [f.lower()])
...
def setup_location_folder_names():
    ...
    for f in folders:
        print f
        rows = mySQL.retrieve_values('media_location_folder', ['name'], [f.lower()])
        if len(rows) == 0:
            mySQL.insert_values('media_location_folder', ['name'], [f.lower()])
...
def clear_bad_entries():
        ...
        data = []
        rows  = mySQL.retrieve_values('problem_esid', ['distinct esid', 'index_name', 'document_type'], [])
