...
import config, start, mySQL, esutil
...
def get_discounts():
    discounts = {}
    rows = mySQL.retrieve_values('match_discount', ['target', 'method', 'value'], [config.MEDIA_FILE])
    for row in rows:
...
def get_weights():
    weights = {}
    rows = mySQL.retrieve_values('match_weight', ['target', 'pattern', 'value'], [config.MEDIA_FILE])
    for row in rows:
...
def get_matches(esid, reverse=False, union=False):
    ...
    if reverse: return mySQL.run_query(query['reverse'] + order_clause)
    elif union: return mySQL.run_query(query['union'] + order_clause)
    else: return mySQL.run_query(query['match'] + order_clause)
...
def get_media_files(path, reverse=False, union=False):
    ...
    if reverse: return mySQL.run_query(query['reverse'])
    elif union: return mySQL.run_query(query['union'])
    else: return mySQL.run_query(query['match'])
